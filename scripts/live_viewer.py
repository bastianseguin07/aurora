...
tkinter/GTK). Muestra siempre la nube base (gris) y, opcionalmente, la nube
...
Tambien soporta fijar la base de comparacion a partir del propio frame en
vivo del sensor ("Fijar BASE en vivo"), sin necesidad de guardar un .ply
intermedio primero - util para monitoreo continuo en terreno.
"""
...
class LiveViewer:
    ...
    def set_live_filters(
        ...
        """Ajustes de captura en vivo: rango maximo, angulo de cono (FOV), eje
        frontal del sensor, e inversion de ejes para adaptarse al montaje."""
        with self._lock:
    ...
    def capture_live_baseline(self) -> int:
        """Usa el ultimo frame en vivo como nueva referencia para medir distancias."""
        with self._lock:
    ...
    def clear_live_baseline(self) -> None:
        """Vuelve a usar la nube base original (la cargada al abrir la vista)."""
        with self._lock:
    ...
    def get_live_snapshot_clouds(self) -> tuple[o3d.geometry.PointCloud | None, o3d.geometry.PointCloud | None, bool]:
        """Devuelve (base_referencia, frame_actual, usando_base_en_vivo) para guardar a disco."""
        with self._lock:
            if self._last_live_points is None or len(self._last_live_points) == 0:
